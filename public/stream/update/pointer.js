...
NewPointerUpdate.prototype.getImageData = function (pointerCtx) {
  ...
  for (let y = 0; y < this.height; y++) {
    ...
    for (let x = 0; x < this.width; x++) {
      ...
      if (andPixel) {
        if (xorPixel === 0x000000ff) /* black -> transparent */ xorPixel = 0x00000000
...
NewPointerUpdate.prototype.getImageData1Bpp = function (pointerCtx) {
  ...
  for (let y = 0; y < this.height; y++) {
    ...
    for (let x = 0; x < this.width; x++) {
      ...
      if (!andPixel && !xorPixel) color = 0xff000000 /* black */
      else if (!andPixel && xorPixel) color = 0xffffffff /* white */
      else if (andPixel && !xorPixel) color = 0x00000000 /* transparent */
      else if (andPixel && xorPixel) color = invertedPointerColor(x, y) /* inverted */
