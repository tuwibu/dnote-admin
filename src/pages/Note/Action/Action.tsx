...
import { API, TABLE_NAME } from '../constant'
  ...
  const handleDelete = async () => {
    try {
      ...
      if (ids && ids.length > 0) {
        ...
        message.open({
          type: 'success',
          content: `Deleted ${successfulDeletes.length} ${TABLE_NAME.toLowerCase()} successfully`
        })
        reload()
      } else {
        message.open({
          type: 'error',
          content: `No ${TABLE_NAME.toLowerCase()} selected to delete`
        })
      }
    } catch (ex) {
      message.open({
